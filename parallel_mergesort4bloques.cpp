	...
	#pragma omp parallel
	{
		#pragma omp single
		{
			...
			#pragma omp task depend(in:c,d) depend(out:f)
			{
				...
				merge(arr, 2*q+1, n-1, 3*q+1);
				f=6;
			}
		}
	}
	merge(arr, 0, n-1, 2*q+1);
	/********************************/
